...
abstract class Shortcode implements \JsonSerializable {
	...
	/**
	 * Attribute labels, leave empty to auto generate.
	 *
	 * @var array
	 */
	...
	protected function _setSchemaAttribute($name, $value = '', $type = 'text', array $extra = array()) {
		...
		// retrieve field label from extra array
		if(array_key_exists('label', $extra)) {
			$attribute['label'] = $extra['label'];
			unset($extra['label']);
		}
		
		// retrieve field label from the labels property (overrides extra)
		if(array_key_exists($name, $this->_labels) && $this->_labels[$name]) {
			$attribute['label'] = $this->_labels[$name];
		}
	...
	/**
	 * Renders the shortcode code with the provided attributes and content.
	 *
	 * @param array $attributes
	 * @param string $content
	 * @return string
	 */
	public function getShortcode($attributes = array(), $content = '') {
		$attributes = shortcode_atts($this->_attributes, $attributes);
		$attribute_list = '';
		...
		$attribute_list = implode(' ', $attribute_list);
		$tag_name = $this->getTagName();
		$code = "[{$tag_name} /]";
		
		if($this->_enclosed) {
			if($attribute_list) {
				$code = sprintf('[%s %s]%s[/%s]', $tag_name, $attribute_list, $content, $tag_name); 
			}
			else {
				$code = sprintf('[%s]%s[/%s]', $tag_name, $content, $tag_name);
			}
		}
		else if($attribute_list) {
			$code = sprintf('[%s %s /]', $tag_name, $attribute_list);
		}
		
		$code = apply_filters('sui_shortcode_get', $code, $this, $attributes, $content);
	...
	public function toArray() {
		...
		$features = apply_filters('sui_shortcode_array', $features, $this);
