...
window.sui.Dialog = function() {
	...
	/**
	 * Returns a underscore template that is
	 * parsed using different delimiters:
	 *
	 * interpolate (print):	{%= value_name %}
	 * evaluate (execute):	{% code_to_execute %}
	 * escape:				{%- <script> %}
	 * 
	...
	this.template = function(content) {
		var original = jQuery.extend(true, {}, _.templateSettings),
			template;
	...
	this.renderShortcodeSelectField = function() {
		...
		// parse template
		template = this.template(window.sui.templates['sui-select']);
	...
	this.renderShortcodeFormFields = function(shortcode) {
		...
		// parse template
		template = this.template(window.sui.templates['sui-form']);
